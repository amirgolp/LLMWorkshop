...
def print_keys(data, indent=''):
    ...
    for key, value in data.items():
        ...
        if isinstance(type, list):
            ...
            values = [my_random_string(10), my_random_string(10)]
        elif type == 'bool':
            values = ['True', 'False']
        elif type == 'int' or type == 'int32' or type == 'int64':
            values = [str(random.randint(1000)), str(random.randint(1000))]
        elif type == 'float' or type == 'float32' or type == 'float64':
            values = ["{:.5f}".format(random.randint(10) * random.random()), "{:.5f}".format(random.randint(10) * random.random())]
        elif type.startswith('<nomad.metainfo.metainfo._Datetime'):
            values = [random_date_generator(f'2010-03-14T15:0{str(random.randint(9))}:20.36', 1000).astype(str), random_date_generator(f'2010-03-14T15:0{str(random.randint(9))}:32.21', 500).astype(str)]
                ...
                for val in values:
                    for query_type in query_types:
                        ...
                        if query_type == '':
                            ...
                            multi_val = rnd.sample(values, random.randint(2, min(len(values), 3) + 1))
                            if query_type == ':any':
                                prompt = raw_prompt + f' with one of the values {multi_val}'
                            else:
                                prompt = raw_prompt + f' with all the values {multi_val}'
